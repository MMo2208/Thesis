...
\ \\
This Bachelor Thesis approaches a web application, which will be running on a Linux Virtual-Machine but executing tasks on the mainframe's \ac{OS} called z/OS. By building this bridge between the Linux VM and the z/OS, students will be able to get in touch with the Mainframe located in Frankfurt University through a web-browser and as a consequence experience how the system is working.\\
\ \\
...
The most common utilization of the mainframe is divided into two categories: 
\begin{itemize}
...
To define the target group of this thesis in more detail, this chapter will give a quick overview on the future users. For this, a few staff-roles will be explained, which are separated in their duties in large mainframe installations. \\
Kettner et al. \cite{kettner2011introduction} distinguish between the "members of the system programming staff" and the "system programming department and other IT departments in the IT organisation" and describe this separation as an "audit requirement" that is to avoid having one person with too much power in the system.\\
A few of the duties that are described by IBM \cite{ebbers2016introduction} are the following:
\begin{itemize}
\item \textbf{System programmers} - install, customize, and maintain the operating system, and also install or upgrade products that run on the system
\item \textbf{System administrators} - perform more of the day-to-day tasks related to maintaining the critical business data that resides on the mainframe (for example, DBA, storage, network, security, and performance)
\item \textbf{Application designers and programmers} - design, build, test, and deliver mainframe applications for the company's end users and customers
\item \textbf{System operators} - monitor and control the operation of the mainframe hardware and software
\item \textbf{Production control analysts} - making sure that batch workloads run to completion - without error or delay
\end{itemize}
\ \\
So the main focus here is on the the \textit{production control analysts} while the keywords are \textit{without error or delay}. The upcoming program will help future users to strengthen their abilities to write batch jobs till they are ready to work this out in a large mainframe installation without making any mistakes.
\\
\subsection{Mainframe Operation Systems}
Another ambit where insights are required for further understanding of the application is the ambit of the different Operation Systems that can be run on the \textit{z-Series}.\\
IBM \textit{z-Series} describes a mainframe architecture for large scale systems where the \textit{z} stands for \textit{zero Downtime}.\\
The following operating systems are designed for this kind of mainframe architecture: 
\ \\
\begin{itemize}
\item \textbf{z/OS} is the IBM operating system that runs on most z-system mainframe installations and by that takes the leading role in the field of mainframes. It isn't set apart a lot from normal operating systems as it has the same structure of three layers, consisting of Hardware, Operating System and User Processes. Similar to modern windows-systems there are some kind of subsystems, which run between the OS and the user processes. The most important one in the case of z/OS are the \ac{JES} for the background operations (batch processing), the \ac{TSO} for the foreground operations (interactive) and the \ac{USS}, which represent a posix-compatible unix subsystem.
\item \textbf{z/VSE} is the system that is used for \ac{OLTP}. It is based on web solutions to integrate OLTP in a heterogeneous environment.
\item \textbf{z/TPF} works as a transaction processor and a database in one operating system. It is used as OS for high data-throughput as needed for example in airline IT-systems where it also has to be available 24/7.
\item \textbf{Linux for zSeries} is a special Linux OS, compiled to be run on IBM mainframes, especially the IBM z-Series and the IBM LinuxONE servers.
\item \textbf{z/VM} This system allows running thousands of distributed servers on one server at a time, which becomes possible through its huge virtualization capabilities. With its two components, a \ac{CP} and a single-user operating system (a \ac{CMS}) it can run any of the IBM operating systems mentioned above but also guest systems such as Ubuntu, which will be the operating system to be used as a server for this application.
\end{itemize}
\ \\
\newpage
...
While the web-access to z/OS for OLTP is very established because of the usage, that is often conducted by end-users or computers, that are not located in immediate proximity to the server, the access to the JES-spool through web-applications is less common, since the production control analysts are usually working in ultimate contact to the mainframe.\\
The future users therefore shell be able to access a front-end running on a server that is located in the mainframe and from here execute tasks that take place on this mainframes' z/OS.\\
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.5]{img/userscheme.jpg}
	\caption{Scheme of usage}
	\label{userscheme}
\end{figure}
\\
...
To gain added value on an educational basis there will be some features that help the users understand the functions of the Batch-jobs as well as writing the jobs.\\ \\
This setup will have the ambitions of getting people into the \ac{JCL} quite quickly on the one hand and give them the opportunity to train their abilities and, in this way reduce potential uncertainties towards working on large-scale systems, on the other hand. With this method a pertinent and long-lasting learning effect is to be achieved in short time.\\
\newpage
...
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.2]{img/JCL-1.jpg}
...
\end{enumerate}
\ \\
The z/OS has written (hard coded) application programs which are not associated with any physical resources. Those programs are just names (e.g. IEBGENER), which include internal file-names. Through JCL those programs can be opened for reading and writing during execution.\\
...
The job creation is defined of its structure. In general a job consists of two major parts -the job card, also known as job statement and the job steps or also called job body. They are all introduced with a double slash.
\subsubsection{The Job-statement}
The job statement identifies the job as JCL and calls the unit of work for the operating system to perform.\\
...
Within a job, there are working executions introduced through an \textit{EXEC-statement}. Every job needs at least one execution, but there can obviously be many more in addition. If a job has no execution it stops.
\ \\
\begin{lstlisting}[language=PHP]
//STEP0001 EXEC PGM=IEBGENER
...
//STEP0006 EXEC PROC=PRDPROC1
//STEP0007 EXEC PROC=PRDPROC2
\end{lstlisting}

\subsubsection{DD-Statements}
Through \ac{DD} statements the data-sets that are used while running the program are defined. There has to be a DD statement for every data set that is used or created within a job.
\ \\
...
/*
//SYSUT2 DD DISP=(NEW,CATLG),DSN=<USERID>.HELLO.DATA
\end{lstlisting}

\begin{itemize}
\item\textbf{SYSIN} Specifies the control dataset
\item\textbf{SYSPRINT} Specifies a message output dataset
\item\textbf{SYSUT1} Specifies the input dataset(s)
\item\textbf{SYSUT2} Specifies the output dataset
\end{itemize}
\ \\
As it can be seen here the control datasat is defined as a dummy. After that there is a message output dataset defined with the input \textit{Hello World!} that is displayed afterwards.

\subsection{The Job Entry Subsystem}
The purpose of the Job Entry Subsystem is to automate the batch jobs. It is a special system component provided by the z/OS which is responsible for controlling and processing all the jobs running on the system (incl. TSO).\\ \\
The JES therefore handles the JCL in three steps as follows:
\begin{itemize}
\item \textbf{Preprocessing} Reading + interpreting the JCL-records and providing the required datasets
\item \textbf{Processing} Performing the job steps
\item \textbf{Postprocessing} The data created through the job gets printed or it gets supplied to the user for further processing
\end{itemize}
\ \\
So while the JES is that part of z/OS which is responsible for reading and processing the JCL that shall be transmitted from the front-end it should work as a contact system to be addressed by our application. The next chapters will introduce the way how to contact the Job-Entry-Subsystem to be able to submit jobs directly to it.
\newpage
...
To enable the communication between different applications, a protocol is needed. The protocol that is used in the System Z is the \ac{TCP/IP}, which is provided through the HiperSockets network. HiperSockets is a technology developed by IBM to enable high-speed communication between \ac{LPAR} with a hypervisor or between applications inside a z/VM.\\
You could imagine a HiperSockets-network like an internal \ac{LAN}, linking all partitions for internal communication.\\
\begin{figure}[h]
	\centering
	\includegraphics[scale=0.6]{img/hipersockets.jpg}
	\caption{Hipersockets}
	\label{Hipersockets}
\end{figure}
\ \\
\subsection{Access via FTP-Server}
...
With the help of a Java program you can use the FTP server to get access to a number of JES functions, including the following:
\begin{itemize}
...
\end{itemize}
\ \\
But although some people and also some companies use this method to contact their mainframe from a Linux host or another type of server, it is not very sophisticated due to the handling and its connection to modern web technologies on the one hand but more important due to the lack of safety on the other hand as the FTP connection alone is not encrypted. \\

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.58]{img/unsecFTP.jpg}
	\caption{Unsecure communication}
	\label{unsecFTP}
\end{figure}

While this application is to be used by many different users it is not recommendable to establish an unencrypted connection to a universities supercomputer.

\subsection{Access via \ac{SSH}/ \ac{SFTP}}
To get a safe connection, it needs a way of encryption what in this case can be established through the SSH-protocol - making the FTP a Secure File Transfer Protocol.\\
The SSH protocol which is also known as Secure Shell gives the opportunity of a remote login from any kind of a computer onto another. With it there are many options for a safe authentication and it encrypts the communication strongly. Combined with the transfer through the FTP Server it is known as SFTP - in this case the perfect way to have a strong authentication and transfer the job files in a safety manner.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.58]{img/secFTP.jpg}
	\caption{SSH encrypted communication}
	\label{secFTP}
\end{figure}
What has to be done here as it can be seen on Fig. \ref{secFTP} is the implementation of an encrypted SSH-Tunnel. This can be achieved by an SSH/SFTP - Server / Client that runs on top of the FTP Server and from here handles the authentication and the security of the communication.\\
The next chapter will introduce two possible solutions that are working on system Z.

\subsection{Choosing an SFTP-Server}
With the research of possible solutions for this case it became quickly clear, that this market is not really flooded. Basically there are only two companies dealing with this issue of getting a safe communication between a server and a mainframe on the backend side. These two are \textit{SSH Communications Security} and \textit{Dovetailed Technologies ,LLC}. \\ \\
The \textit{SSH Communications Security} is a Finnish company founded by the inventor of the SSH protocol, Tatu Ylonen. With its subcompany \textit{Tectia} it provides several solutions for business cases to establish an SSH connection in large mainframe systems and here - of course - also has a solution for the IBM Mainframe. While holding the patents for the algorithms behind the SSH protocol you can imagine that their product is quite costly and therefore stays a "business case".\\ \\
\textit{Dovetailed Technologies} on the other hand uses the openSSH protocol for their work and offers their software for "non-business-cases" for free. OpenSSH is an implementation of the SSH-protocol developed by openBSD as an openSource project and does basically the same as the SSH-protocol. You can imagine openSSH as a meal, cooked by the receipt of the SSH-protocol, that can now be used to implement an encrypted connection just as it is needed for this application. \\ \\
Therefore the \textit{Co:Z SFTP-Server}, as the product of Dovetailed Technologies is called, will be our entry point to the JES-Spool in this application.


\subsection{Co:Z construction}
Along with some other products the \textit{Co:Z SFTP-Server} is a tool of the before mentioned company \textit{Dovetailed Technologies LLC}. They describe the function of their products based on the \textit{Co:Z Hybrid Batch} on their website. The hybrid batch is basically the same as the SFTP-Server but the other way around. With the hybrid batch you can control actions on a linux LPAR for example from the mainframe. Figure \ref{hybbatch} is just to show, that the construction of their products is exactly what is needed for the Mainframe Self-Service.

\begin{figure}[h]
	\centering
	\includegraphics[scale=0.45]{img/hybrid_batch_website.png}
	\caption{Hybrid Batch Construction, Source: dovetail.com}
	\label{hybbatch}
\end{figure}

On the one hand there is pure data that can be the output or the input, which is transferred encrypted throughout the zEnterprise and other operation systems, that even don't have to be located in the mainframe here. The \textit{Co:Z Launcher} that can be seen here is also a part of the Co:Z SFTP-Server because its function is to work as a target system on the mainframe translate the incoming data so that it can be send to another operation system or in our case gets the input and forwards it to the Job Entry Subsystem. As Fig. \ref{hybbatch} also reveals, the technology for the communication inside the zEnerprise are the hypersockets that run the TCP/IP protocol. \\ \\
With the Co:Z Launcher as a target system for accessing the JES and the Co:Z SFTP Server running to get an encrypted communication between the systems, it is now possible to submit spool files to the mainframe and with this create batch-jobs and get direct response.\\ \\
But this is only possible through a terminal so far. The next chapters therefore will carry out how this tool can be used inside a website that gives the users the possibility to simply write JCL, have it submitted and get an overview on how they are doing with this without using a terminal.\\
\newpage
...
Now that the technologies for the application are set, it is time to get them together. To get access to z/OS, a system constellation as seen on Fig. \ref{constellation} will be implemented. \\
A z/VM running Linux (LPAR 1) will serve as a web-server. Here the application is hosted and can be accessed through the user's web-browser. Due to the hipersockets network, that links LPAR 1 to the z/OS (LPAR 2), whereupon an SFTP-Server can be run, a stable and safe connection can be established. As before mentioned, the Co:Z SFTP-Server is deployed here, to get direct access to the JES-Spool. Through this constellation, the users will be able to submit jobs directly on the mainframe, from any given device that they are using to access the internet.\\
\begin{figure}[h]
	\centering
	...
	\label{constellation}
\end{figure} \\
To be independent from the users platform this application is developed as a website that can simply be accessed by any modern browser as already said. Since the application here is accessed through the front-end but operates on the back-end where the mainframe is contacted, a programming language is needed, that can run on the front-end but on the back-end as well. The choice for this is javascript. More precisely the \textit{node.js} platform will be used. Node.js is a serverside platform to run web applications written in javascript. Due to its huge variety of predefine packages, that can be used here to realize many different kind of tasks it is perfect for developing this application.
...
To access z/OS via SFTP we use the ssh2-package by requiring it on line 1. Then we establish a variable \textit{connSettings} which contains all the requirements to access the mainframes' z/OS through SFTP and get access to the JES-Spool to read and write data.\\
\begin{lstlisting}[language=Java]
...
Since the application in first place is designed to give students and production control analysts in general the ability to learn submitting batch-jobs, the functionality itself is not enough. This chapter will describe three techniques to give the user a learning effect. \\
